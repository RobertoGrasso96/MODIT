...
public final class LongArrays {
	...
	protected static class ForkJoinQuickSort2 extends RecursiveAction {
		...
		protected void compute() {
			...
			while (true) {
				int comparison, t;
				while (b <= c && (comparison = (t = (Long.compare((x[b]), (v)))) == 0
						? (Long.compare((y[b]), (w)))
						: t) <= 0) {
					if (comparison == 0)
						swap(x, y, a++, b);
				...
				while (c >= b && (comparison = (t = (Long.compare((x[c]), (v)))) == 0
						? (Long.compare((y[c]), (w)))
						: t) >= 0) {
					if (comparison == 0)
						swap(x, y, c, d--);
	...
	public static void mergeSort(final long a[], final int from, final int to, long supp[]) {
		int len = to - from;
	 ...
	 *            cloned.
	 */
	public static void mergeSort(final long a[], final int from, final int to, LongComparator comp, long supp[]) {
		int len = to - from;
		// Insertion sort on smallest arrays
		if (len < MERGESORT_NO_REC) {
