...
public final class CharArrays {
	...
	public static void quickSort(final char[] x, final char[] y, final int from, final int to) {
		...
		while (true) {
			int comparison, t;
			while (b <= c && (comparison = (t = (Character.compare((x[b]), (v)))) == 0
					? (Character.compare((y[b]), (w)))
					: t) <= 0) {
				if (comparison == 0)
					swap(x, y, a++, b);
				b++;
			}
			while (c >= b && (comparison = (t = (Character.compare((x[c]), (v)))) == 0
					? (Character.compare((y[c]), (w)))
					: t) >= 0) {
				if (comparison == 0)
					swap(x, y, c, d--);
				c--;
	...
	public static void mergeSort(final char a[], final int from, final int to, char supp[]) {
		int len = to - from;
	 ...
	 *            cloned.
	 */
	public static void mergeSort(final char a[], final int from, final int to, CharComparator comp, char supp[]) {
		int len = to - from;
		// Insertion sort on smallest arrays
		if (len < MERGESORT_NO_REC) {
	...
	public static void parallelRadixSort(final char[] a, final int from, final int to) {
		if (to - from < PARALLEL_RADIXSORT_NO_FORK || ForkJoinPool.getCommonPoolParallelism() == 1) {
			quickSort(a, from, to);
			return;
		}
		final int maxLevel = DIGITS_PER_ELEMENT - 1;
		...
		final int numberOfThreads = ForkJoinPool.getCommonPoolParallelism();
