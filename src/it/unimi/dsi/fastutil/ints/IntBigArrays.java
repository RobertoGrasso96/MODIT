...
public final class IntBigArrays {
	...
	public static final AtomicIntegerArray[] EMPTY_BIG_ATOMIC_ARRAY = {};
	/**
	 ...
	 */
	public static AtomicIntegerArray[] newBigAtomicArray(final long length) {
		if (length == 0)
		...
		if (residual != 0) {
			for (int i = 0; i < baseLength - 1; i++)
				base[i] = new AtomicIntegerArray(SEGMENT_SIZE);
	...
	private static void swap(final int[][] x, long a, long b, final long n) {
	...
	protected static class ForkJoinQuickSort extends RecursiveAction {
		...
		private final int[][] x;
		public ForkJoinQuickSort(final int[][] x, final long from, final long to) {
			this.from = from;
			this.to = to;
			this.x = x;
		}
	 ...
	 *            the index of the last element (exclusive) to be sorted.
	 */
	public static void parallelQuickSort(final int[][] x, final long from, final long to) {
	...
	protected static class ForkJoinQuickSortComp extends RecursiveAction {
		...
		public ForkJoinQuickSortComp(final int[][] x, final long from, final long to, final IntComparator comp) {
			this.from = from;
			this.to = to;
			this.x = x;
			this.comp = comp;
	 ...
	 *            the comparator to determine the sorting order.
	 */
	public static void parallelQuickSort(final int[][] x, final long from, final long to, final IntComparator comp) {
		if (to - from < PARALLEL_QUICKSORT_NO_FORK || ForkJoinPool.getCommonPoolParallelism() == 1)
			quickSort(x, from, to, comp);
		else {
			final ForkJoinPool pool = ForkJoinPool.commonPool();;
